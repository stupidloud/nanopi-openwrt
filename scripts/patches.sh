...
line_number_INCLUDE_V2ray=$[`grep -m1 -n 'Include V2ray' package/custom/luci-app-passwall/Makefile`-1]
sed -i $line_number_INCLUDE_V2ray'd' package/custom/luci-app-passwall/Makefile
sed -i $line_number_INCLUDE_V2ray'd' package/custom/luci-app-passwall/Makefile
sed -i $line_number_INCLUDE_V2ray'd' package/custom/luci-app-passwall/Makefile
...
if [[ $DEVICE == 'r1s' ]]; then
...
elif [[ $DEVICE == 'r4s' || $DEVICE == 'r1p' || $DEVICE == 'r1p-lts' ]]; then
  ...
  sed -i $line_number_CONFIG_CRYPTO_LIB_BLAKE2S' s/HIDDEN:=1/DEPENDS:=@(LINUX_5_4)/' package/kernel/linux/modules/crypto.mk
fi
