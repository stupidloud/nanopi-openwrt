...
board_id=$(cat /etc/board.json | jsonfilter -e '@["model"].id' | tail -c 4 | tr -d "\n")
...
wget https://ghproxy.com/https://github.com/klever1988/nanopi-openwrt/releases/download/$board_id-$(date +%Y-%m-%d)/$board_id.img.gz -O rom.img.gz
if [ -f rom.img.gz ]; then
	...
	pv rom.img.gz | gunzip -dc > FriendlyWrt.img && rm rom.img.gz
